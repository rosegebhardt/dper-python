...
# # NEW FUNCTION
# @njit(fastmath=True)
# def numba_internal_forces_kernel(
#     z0, N, d_b, d_s, E, a_edge_ref, l_edge_ref, I_node_ref, l_node_ref,
#     centerline_scaling, wave_amplitude, wave_number, wave_frequency, wave_phase,
#     recenterline, u, time, CT, CN, external_velocity, fluid_density, CA, h_edge_ref, m_node_ref
# ):
#     # Manually unpack z0 into x and v according to order='F' layout
#     x = np.zeros((2, N))
#     v = np.zeros((2, N))
#     for i in range(N):
#         x[0, i] = z0[2 * i]
...
#     # Compute edge vectors and unit normals
#     e = np.zeros((2, N-1))
#     l_edge = np.zeros(N-1)
#     t = np.zeros((2, N-1))
#     n = np.zeros((2, N-1))
...
#     for ii in range(N-1):
#         e[0, ii] = x[0, ii+1] - x[0, ii]
#         e[1, ii] = x[1, ii+1] - x[1, ii]
#         l_edge[ii] = np.sqrt(e[0, ii]**2 + e[1, ii]**2)
#         t[0, ii] = e[0, ii] / l_edge[ii]
#         t[1, ii] = e[1, ii] / l_edge[ii]
#         n[0, ii] = -t[1, ii]
#         n[1, ii] =  t[0, ii]
...
#         dot = t[0, ii-1] * t[0, ii] + t[1, ii-1] * t[1, ii]
#         phi[ii] = np.arctan2(cross, dot)

#     kappa = 2.0 * np.tan(phi[1:] / 2.0)
#     angle_ref = centerline_scaling * wave_amplitude * (
#         np.sin(wave_number * recenterline + wave_frequency * time + wave_phase) + u
#     )
#     kappa_ref = 2.0 * np.tan(angle_ref / 2.0)

#     # Elastic Forces
#     f_stretch = E * a_edge_ref * (l_edge / l_edge_ref - 1.0)
#     FS = np.zeros((2, N))
#     for ii in range(N-1):
#         FS[0, ii] += f_stretch[ii] * t[0, ii]
#         FS[1, ii] += f_stretch[ii] * t[1, ii]
#         FS[0, ii+1] -= f_stretch[ii] * t[0, ii]
#         FS[1, ii+1] -= f_stretch[ii] * t[1, ii]

#     cos_denom = np.maximum(1.0 + np.cos(phi[1:]), 1e-4)
#     bending_partials = (0.2 * E * I_node_ref[1:-1] * (kappa - kappa_ref)) / (l_node_ref[1:-1] * cos_denom)

#     f_bending = np.zeros(N-1)
#     f_bending[0] = bending_partials[0] / l_edge[0]
#     for ii in range(1, N-2):
#         f_bending[ii] = (-bending_partials[ii-1] + bending_partials[ii]) / l_edge[ii]
#     f_bending[N-2] = -bending_partials[-1] / l_edge[N-2]

#     FB = np.zeros((2, N))
#     for ii in range(N-1):
#         FB[0, ii] += f_bending[ii] * n[0, ii]
#         FB[1, ii] += f_bending[ii] * n[1, ii]
#         FB[0, ii+1] -= f_bending[ii] * n[0, ii]
#         FB[1, ii+1] -= f_bending[ii] * n[1, ii]

#     FE = FS + FB

#     # Dissipative Forces
#     cos_denom_all = np.maximum(1.0 + np.cos(phi[1:N-1]), 1e-4)
#     kappa_dot_scale = 2.0 / cos_denom_all
    
#     kappa_dot_dir = np.zeros(N-2)
#     for ii in range(N-2):
#         term1 = (n[0, ii]*v[0, ii] + n[1, ii]*v[1, ii]) / l_edge[ii]
#         term2 = ((n[0, ii] + n[1, ii+1])*v[0, ii+1] + (n[1, ii] + n[1, ii+1])*v[1, ii+1]) / l_edge[ii+1]
#         term3 = (n[0, ii+1]*v[0, ii+2] + n[1, ii+1]*v[1, ii+2]) / l_edge[ii+1]
#         kappa_dot_dir[ii] = term1 - term2 + term3

#     kappa_dot = np.zeros(N)
#     kappa_dot[1:N-1] = kappa_dot_scale * kappa_dot_dir

#     FBD = np.zeros((2, N))
#     c_phi = np.maximum(1.0 + np.cos(phi), 1e-4)
    
#     FBD[:, 0] = d_b * (-2.0 * kappa_dot[1] / c_phi[1]) * n[:, 0] / l_edge[0]
#     FBD[:, 1] = (d_b * (2.0 * kappa_dot[1] / c_phi[1]) * n[:, 0] / l_edge[0] +
#                  d_b * (2.0 * kappa_dot[1] / c_phi[1] - 2.0 * kappa_dot[2] / c_phi[2]) * n[:, 1] / l_edge[1])
    
#     for jj in range(2, N-2):
#         FBD[:, jj] = (
#             d_b * (-2.0 * kappa_dot[jj-1] / c_phi[jj-1] + 2.0 * kappa_dot[jj] / c_phi[jj]) * n[:, jj-1] / l_edge[jj-1] +
#             d_b * (2.0 * kappa_dot[jj] / c_phi[jj] - 2.0 * kappa_dot[jj+1] / c_phi[jj+1]) * n[:, jj] / l_edge[jj]
#         )
    
#     FBD[:, N-2] = (d_b * (-2.0 * kappa_dot[N-3] / c_phi[N-3] + 2.0 * kappa_dot[N-2] / c_phi[N-2]) * n[:, N-3] / l_edge[N-3] +
#                    d_b * (2.0 * kappa_dot[N-2] / c_phi[N-2]) * n[:, N-2] / l_edge[N-2])
#     FBD[:, N-1] = d_b * (-2.0 * kappa_dot[N-2] / c_phi[N-2]) * n[:, N-2] / l_edge[N-2]

#     # Stretching Dissipative
#     FSD = np.zeros((2, N))
#     diff_v0 = v[:, 1] - v[:, 0]
#     proj0 = (t[0, 0]*diff_v0[0] + t[1, 0]*diff_v0[1]) * t[:, 0]
#     FSD[:, 0] = d_s * proj0

#     for jj in range(1, N-1):
#         diff_next = v[:, jj+1] - v[:, jj]
#         proj_next = (t[0, jj]*diff_next[0] + t[1, jj]*diff_next[1]) * t[:, jj]
#         diff_prev = v[:, jj] - v[:, jj-1]
#         proj_prev = (t[0, jj-1]*diff_prev[0] + t[1, jj-1]*diff_prev[1]) * t[:, jj-1]
#         FSD[:, jj] = d_s * (proj_next - proj_prev)

#     diff_last = v[:, N-1] - v[:, N-2]
#     proj_last = (t[0, N-2]*diff_last[0] + t[1, N-2]*diff_last[1]) * t[:, N-2]
#     FSD[:, N-1] = -d_s * proj_last

#     FID = FBD + FSD

#     # Hydrodynamic Drag
#     v_ext = (external_velocity[:, 0:N-1] + external_velocity[:, 1:N]) / 2.0
#     v_edge = (v[:, 0:N-1] + v[:, 1:N]) / 2.0
#     v_rel = v_edge - v_ext

#     FHD_edge = np.zeros((2, N-1))
#     for ii in range(N-1):
#         v_T = v_rel[0, ii] * t[0, ii] + v_rel[1, ii] * t[1, ii]
#         v_N = v_rel[0, ii] * n[0, ii] + v_rel[1, ii] * n[1, ii]
#         FHD_edge[:, ii] = -CT[ii] * (v_T * np.abs(v_T)) * t[:, ii] - CN[ii] * (v_N * np.abs(v_N)) * n[:, ii]

#     FHD = np.zeros((2, N))
#     FHD[:, 0] = FHD_edge[:, 0] / 2.0
#     for ii in range(1, N-1):
#         FHD[:, ii] = (FHD_edge[:, ii-1] + FHD_edge[:, ii]) / 2.0
#     FHD[:, N-1] = FHD_edge[:, N-2] / 2.0

#     FTotal = FE + FID + FHD
    
#     # Interleaved FOther creation (matching order='F')
#     FOther = np.zeros(2 * N)
#     for i in range(N):
#         FOther[2 * i] = FTotal[0, i]
#         FOther[2 * i + 1] = FTotal[1, i]

#     # Added Mass Matrix
#     mu_normal = fluid_density * np.pi * CA * l_edge_ref * (h_edge_ref**2) / 4.0
#     M = np.zeros((2*N, 2*N))

#     n00 = np.outer(n[:, 0], n[:, 0])
#     M[0:2, 0:2] = 0.25 * mu_normal[0] * n00 + m_node_ref[0] * np.eye(2)
#     M[0:2, 2:4] = 0.25 * mu_normal[0] * n00

#     for jj in range(1, N-1):
#         njm1 = np.outer(n[:, jj-1], n[:, jj-1])
#         nj = np.outer(n[:, jj], n[:, jj])
#         M[2*jj:2*jj+2, 2*jj-2:2*jj] = 0.25 * mu_normal[jj-1] * njm1
#         M[2*jj:2*jj+2, 2*jj:2*jj+2] = m_node_ref[jj] * np.eye(2) + 0.25 * mu_normal[jj-1] * njm1 + 0.25 * mu_normal[jj] * nj
#         M[2*jj:2*jj+2, 2*jj+2:2*jj+4] = 0.25 * mu_normal[jj] * nj

#     nNM2 = np.outer(n[:, N-2], n[:, N-2])
#     M[2*N-2:2*N, 2*N-4:2*N-2] = 0.25 * mu_normal[N-2] * nNM2
#     M[2*N-2:2*N, 2*N-2:2*N] = 0.25 * mu_normal[N-2] * nNM2 + m_node_ref[N-1] * np.eye(2)

#     accel = np.linalg.solve(M, FOther)

#     dzdt = np.empty(4*N)
#     dzdt[0:2*N] = z0[2*N:4*N]
#     dzdt[2*N:4*N] = accel
...
class Fish:
    ...
    #     return numba_internal_forces_kernel(
    #         z0, self.N, self.d_b, self.d_s, self.E, self.a_edge_ref,
    #         self.l_edge_ref, self.I_node_ref, self.l_node_ref,
    #         self.centerline_scaling, self.wave_amplitude, self.wave_number,
    #         self.wave_frequency, self.wave_phase, self.recenterline, u, time,
    #         self.CT, self.CN, self.external_velocity, self.fluid_density,
    #         self.CA, self.h_edge_ref, self.m_node_ref
    #     )
