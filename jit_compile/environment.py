...
def numba_fvs_velocity_kernel(x, y, v_x, v_y, gamma, delta):
    """
    Computes velocity contribution of free vortices on target points (x, y).
    """
    n_targets = x.shape[0]
    n_vortices = v_x.shape[0]
    
    u_x = np.zeros(n_targets, dtype=np.float64)
    u_y = np.zeros(n_targets, dtype=np.float64)
    ...
    # Parallel loop over all target points
    for i in prange(n_targets):
        ...
        for j in range(n_vortices):
            ...
            strength = gamma[j] / two_pi
            inv_norm2 = strength / norm2
...
@njit(fastmath=True, parallel=True)
def numba_bvs_velocity_kernel(x, y, v_x, v_y, gamma, length, delta):
    """
    Computes velocity contribution of bound vortices on target points (x, y).
    """
    n_targets = x.shape[0]
    n_vortices = v_x.shape[0]
    ...
    for i in prange(n_targets):
        ...
        for j in range(n_vortices):
            ...
            strength = (gamma[j] * length[j]) / two_pi
            inv_norm2 = strength / norm2
...
class Environment:
    ...
    def fvs_flow_velocity(self, z, delta=0.2):
        x = np.ascontiguousarray(z[0, :], dtype=np.float64)
        y = np.ascontiguousarray(z[1, :], dtype=np.float64)

        u_x = np.zeros_like(x)
        u_y = np.zeros_like(y)

        for ii in range(self.fish_N):
            if self.fvs_positions[ii].shape[1] == 0:
                continue

            v_x = np.ascontiguousarray(self.fvs_positions[ii][0, :], dtype=np.float64)
            v_y = np.ascontiguousarray(self.fvs_positions[ii][1, :], dtype=np.float64)
            gamma = np.ascontiguousarray(self.fvs_Gamma[ii], dtype=np.float64)

            ux_i, uy_i = numba_fvs_velocity_kernel(x, y, v_x, v_y, gamma, float(delta))
            u_x += ux_i
            u_y += uy_i
    ...
    def bvs_flow_velocity(self, z, delta=0.01):
        ...
        for fish in self.fishies:
            v_x = np.ascontiguousarray(fish.bvs_positions[0, :], dtype=np.float64)
            v_y = np.ascontiguousarray(fish.bvs_positions[1, :], dtype=np.float64)
            gamma = np.ascontiguousarray(fish.bvs_gamma, dtype=np.float64)
            length = np.ascontiguousarray(fish.bvs_length, dtype=np.float64)

            ux_i, uy_i = numba_bvs_velocity_kernel(x, y, v_x, v_y, gamma, length, float(delta))
            u_x += ux_i
            u_y += uy_i
